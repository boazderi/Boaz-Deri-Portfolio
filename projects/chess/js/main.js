...
function getAllPossibleCoordsRook(pieceCoord) {
    // get the piece coords object, and return all available places in array
    var res = []
    // check if the rook is stuck by check all diractions
    var posibleDirections = checkStreightDiractions(pieceCoord)
    console.log('posibleDirections', posibleDirections)
    if (!posibleDirections.length) return res
    // if not stuck:
    for (var i = 0; i < posibleDirections.length; i++) {
        var currCells
        switch (posibleDirections[i]) {
            case 'right':
                currCells = checkAllDirectionCells(pieceCoord, "right")
                break;
            case 'left':
                currCells = checkAllDirectionCells(pieceCoord, "left")
                break;
            case 'up':
                currCells =checkAllDirectionCells(pieceCoord, "up")
                break;
            case 'down':
                currCells = checkAllDirectionCells(pieceCoord, "down")
    ...
    // if one of the option is not stack call to the function that return array
    // of all the streight optional cells
